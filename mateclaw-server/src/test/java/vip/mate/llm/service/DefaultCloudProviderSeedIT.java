...
class DefaultCloudProviderSeedIT {
    ...
    // =========================================================================
    // 断言5: 默认 embedding 模型改挂到带平台 key 的 dashscope-default，开箱可用
    //        （原版 dashscope 为空时，embedding 仍能用，因 V145 把 embedding 改挂到 dashscope-default）
    // =========================================================================

    @Test
    @DisplayName("默认工作区的默认 embedding 模型挂在 dashscope-default 上，且该 provider 可用")
    void defaultEmbeddingModel_isHostedOnDashscopeDefault_andUsable() {
        // 默认工作区唯一 is_default=true 的 embedding 行，其 provider 必须已从原版 dashscope 改挂到 dashscope-default
        List<ModelConfigEntity> defaults = jdbcTemplate.query(
                "SELECT provider, model_name FROM mate_model_config " +
                "WHERE workspace_id = 1 AND is_default = TRUE AND deleted = 0 AND model_type = 'embedding'",
                (rs, rowNum) -> {
                    ModelConfigEntity e = new ModelConfigEntity();
                    e.setProvider(rs.getString("provider"));
                    e.setModelName(rs.getString("model_name"));
        ...
        assertEquals(1, defaults.size(),
                "默认工作区必须有且仅有 1 个 is_default=true 的 embedding 模型，实际: " + defaults);
        assertEquals("dashscope-default", defaults.get(0).getProvider(),
                "默认 embedding 模型必须改挂到带平台 key 的 dashscope-default（而非原版空 key 的 dashscope），否则新工作区知识库 embedding 不可用");

        // 该 provider 必须已配置（持有平台 key），保证 WikiEmbeddingService.isUsable 通过
        assertTrue(modelProviderService.isProviderConfigured("dashscope-default"),
                "dashscope-default 必须已配置平台 key，embedding 才能开箱可用");
    }
}
